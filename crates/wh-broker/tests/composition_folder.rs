...
///
/// Uses `CARGO_MANIFEST_DIR` to locate the crate directory, then walks up
/// to the workspace root (two levels: crates/wh-broker -> workspace root).
fn subsystem_folder() -> PathBuf {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    // Walk up from crates/wh-broker to the workspace root
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root should exist two levels above CARGO_MANIFEST_DIR");
    workspace_root.join("subsystems").join("llm-wiki")
}
