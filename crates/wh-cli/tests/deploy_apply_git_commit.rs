...
fn apply_creates_git_commit_with_plan_hash() {
    ...
    assert!(commit_msg.contains("Plan:"), "commit message must contain 'Plan:' with plan hash (ADR-003). Got: {commit_msg}");
    assert!(commit_msg.contains("sha256:"), "commit message must contain sha256 plan hash. Got: {commit_msg}");
}
...
fn apply_commit_contains_operator_name_and_change_summary() {
    ...
    assert!(commit_msg.contains("[operator] apply:"), "commit message must follow ADR-003 format: [operator] apply: <summary>. Got: {commit_msg}");
}
...
fn apply_commit_uses_custom_agent_name() {
    ...
    let commit_msg = String::from_utf8_lossy(&git_log_output.stdout);
    assert!(commit_msg.contains("[donna] apply:"), "commit message must use provided agent name. Got: {commit_msg}");
}
