...
fn plan_hash_is_deterministic_for_same_topology_diff() {
    ...
    let hash1 = json1["data"]["plan_hash"]
        .as_str()
        .expect("plan_hash must be a string");
    let hash2 = json2["data"]["plan_hash"]
        .as_str()
        .expect("plan_hash must be a string");

    assert_eq!(
        hash1, hash2,
        "plan_hash must be deterministic for identical topology diffs"
    );
    assert!(
        hash1.starts_with("sha256:"),
        "plan_hash must use sha256 prefix"
    );
}
...
fn plan_hash_is_canonical_independent_of_field_ordering() {
    ...
    assert_eq!(
        hash1, hash2,
        "semantically identical plans must produce the same canonical hash"
    );
}
