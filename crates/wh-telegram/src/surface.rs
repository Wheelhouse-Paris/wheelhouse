...
impl TelegramSurface {
    ...
    pub async fn handle_outgoing(
        ...
        let target_user_id = text_msg.reply_to_user_id.as_deref().ok_or_else(|| {
            TelegramError::StreamError("outgoing message has no reply_to_user_id".into())
        })?;
