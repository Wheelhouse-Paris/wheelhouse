...
impl SocketSend for PubSocket {
    async fn send(&mut self, message: ZmqMessage) -> ZmqResult<()> {
        // Wheelhouse fork note:
        //
        // Upstream zeromq-rs 0.4.1 used `try_send()` here with a noop waker,
        // which silently dropped any message the underlying `FramedWrite` sink
        // could not *immediately* accept. For small messages that never
        // exceeded the TCP socket send buffer this was imperceptible, but for
        // messages larger than a single flush cycle (~300 KiB PDF attachments
        // in our Telegram surface), the second half of the frame would stay
        // buffered in `FramedWrite` and the next send would silently drop
        // because `poll_ready` still returned `Pending`.
        //
        // The fix is to use the proper async `Sink::send(...).await` path so
        // backpressure is respected: we wait until the sink has actually
        // drained enough to accept the full message before returning.
